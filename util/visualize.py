...
def plot_indicator(params, pt_this, pt_gt, img_this, img_gt):
    ...
    # pix_err (pixel_dist)
    x2 = np.square(pt_this[:, 0] - pt_gt[:, 0])
    y2 = np.square(pt_this[:, 1] - pt_gt[:, 1])
...
def plot_iter_info(params, img):
    ...
    str_info = [
        ...
        f'epoch: [{current_epoch:2}/{params.num_epochs:2}]',
    ]
    txt_info = (cv2.FONT_HERSHEY_SIMPLEX, 1, (192, 192, 192), 2)
    for i, ele in enumerate(str_info):
        cv2.putText(img, ele, (400, (i + 1) * 40), *txt_info)
    return img


def plot_offset_info(pts, img):
    p1, p2 = pts['offset_pred'], pts['offset']
    pm = [
    ...
    txt_info = (cv2.FONT_HERSHEY_SIMPLEX, 1, (192, 192, 192), 2)
    for i, ele in enumerate(str_info):
        cv2.putText(img, ele, (400, (i + 3) * 40), *txt_info)
    return img
...
def plot_pt_unsupervised_kernel(params, pts, bevs_cam):
    ...
    for name, img in bevs_cam.items():
        ...
        elif params.visualize_mode != 'inference':
            img = plot_iter_info(params, img)
            img = plot_offset_info(pts, img)
...
def visualize_unsupervised_kernel(params, data):
    ...
    if params.visualize_mode == 'train':
        ...
        if idx != 1 and idx % params.train_vis_iter_frequence != 0:
            return
    elif params.visualize_mode == 'evaluate':
        num = 1
        if params.eval_vis_iter_frequence <= 0:
            return
        if idx != 1 and idx % params.eval_vis_iter_frequence != 0:
            return
    else:
        num = batch_size
    ...
    if params.current_iter == 1 and os.path.exists(sv_dir):
        if params.visualize_mode == 'evaluate':
