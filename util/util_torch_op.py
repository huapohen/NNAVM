...
def RescaleH(H, rescale_size, patch_size):
    ...
    M = (
        torch.Tensor(
            [
                [patch_size[0] / rescale_size[0], 0, 0],
                [0, patch_size[1] / rescale_size[1], 0],
                [0, 0, 1],
            ]
        )
        .view(1, 3, 3)
...
def WarpImages(input_map, H, start, patch_size):
    def bilinear_interpote(
        ...
        batch_pa = torch.gather(
            batch_img, dim=0, index=Ia.unsqueeze(-1).repeat(1, imgc)
        )
        batch_pb = torch.gather(
            batch_img, dim=0, index=Ib.unsqueeze(-1).repeat(1, imgc)
        )
        batch_pc = torch.gather(
            batch_img, dim=0, index=Ic.unsqueeze(-1).repeat(1, imgc)
        )
        batch_pd = torch.gather(
            batch_img, dim=0, index=Id.unsqueeze(-1).repeat(1, imgc)
        )
        ...
        warp_img = (
            wa * batch_pa + wb * batch_pb + wc * batch_pc + wd * batch_pd
        )  # * kw_mask
    ...
    batch_y = (
        torch.repeat_interleave(torch.arange(patch_size[1]), repeats=patch_size[0])
        .view(1, -1, 1)
        .repeat(bs, 1, 1)
        .to(device)
    )
    ...
    goal = torch.cat(
        [batch_x, batch_y, torch.ones_like(batch_x, dtype=torch.float).to(device)], -1
    ).permute(0, 2, 1)
    ...
    new_batch_img = bilinear_interpote(
        input_map.permute(0, 2, 3, 1).view(bs, -1, imgc),
        offset_batch_xy[:, :, 0],
        offset_batch_xy[:, :, 1],
        patch_size,
        bs,
...
def CropPatchFromFull(patch_size, full_img, start, rescale=False):
    ...
    batch_x = torch.arange(pw).repeat(ph).view(1, -1, 1).repeat(bs, 1, 1).to(
        device
    ) + start[:, :, :1].repeat(1, pw * ph, 1)
    batch_y = torch.repeat_interleave(torch.arange(ph), repeats=pw).view(
        1, -1, 1
    ).repeat(bs, 1, 1).to(device) + start[:, :, 1:].repeat(1, pw * ph, 1)
    if rescale:
        new_batch_img = bilinear_interpote(
            full_img.permute(0, 2, 3, 1).view(bs, -1, c),
            batch_x,
            batch_y,
            patch_size,
            bs,
        ...
        Ip = base_y0 + batch_x
        new_batch_img = torch.gather(
            full_img_flatten, dim=0, index=Ip.long().repeat(1, c)
        )

    return new_batch_img.reshape(bs, patch_size[1], patch_size[0], c).permute(
        0, 3, 1, 2
    )
